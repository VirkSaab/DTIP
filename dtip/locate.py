...
def locate_dti_files(input_path: Union[str, Path],
                     output_path: Union[str, Path],
                     protocol_names: list, n_gradients: int,
                     ret_paths: bool = True) -> Union[dict, int]:
    ...
    bval_paths = sorted(
        [p for p in input_path.glob('*') if p.suffix == '.bval'])
    bvec_paths = sorted(
        [p for p in input_path.glob('*') if p.suffix == '.bvec'])
    ...
    dti_series_dict = {  # series must have ['bvec', 'bval', 'nifti']
